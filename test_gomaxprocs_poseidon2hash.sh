...
# 创建日志目录
LOG_DIR="performance_test_logs_poseidon2hash"
mkdir -p "$LOG_DIR"

# 测试的GOMAXPROCS值
GOMAXPROCS_VALUES=(1 24)
...
for tag in "${BUILD_TAGS[@]}"; do
    ...
    for procs in "${GOMAXPROCS_VALUES[@]}"; do
        ...
        # 运行程序并保存输出到日志文件
        {
            ...
            if [ -z "$TAG_FLAG" ]; then
                GOMAXPROCS=$procs go run examples/poseidonhasher/main.go
            else
                GOMAXPROCS=$procs go run $TAG_FLAG examples/poseidonhasher/main.go
            fi
